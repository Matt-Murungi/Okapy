...
import '../../models/chatNotification.dart';
...
class _NotificationsState extends State<Notifications> {
    ...
    channel = WebSocketChannel.connect(
      Uri.parse(
        // 'ws://apidev.okapy.world:8000/chats/${widget.availableJobs.bookingsDetailsModelActive?.booking?.owner?.id}__${widget.authController.userModel?.id}/?token=${userToken.key}'),
          'ws://api.okapy.world:8000/notifications/?token=${userToken.key}'),
    );
      ...
      if(dataMessages['type']=='get_notifications')
        {
          ...
          List<NotificationModel>chats=keys.map((e) => NotificationModel.fromJson2(e)).toList();
          List<String>titles=[];
  ...
  Widget listItem(NotificationModel bookingDetailsModel) {
    DateTime dt = DateFormat('MM/dd/yyyy HH:mm:ss').parse(bookingDetailsModel.created_at!);
    ...
    return ListTile(
        ...
        trailing: Text(DateFormat("dd MMM yyyy HH:mm").format(dt))
    );
  }
  ...
  Widget getListItemByMonth(String month, List<NotificationModel> listItems) {
    return SliverList(
      delegate: SliverChildListDelegate([
        ...
        ...listItems.map((e) => listItem(e)).toList().reversed,
